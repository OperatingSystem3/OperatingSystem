...
- [lab 3 缺页异常和页面置换](#lab-3-缺页异常和页面置换)
  ...
  - [练习2：深入理解不同分页模式的工作原理（思考题）](#练习2深入理解不同分页模式的工作原理思考题)
    - [2.1 `get_pte()`函数中的类似代码](#21-get_pte函数中的类似代码)
    - [2.2 get\_pte()函数将页表项的查找和页表项的分配合并在一个函数的分析](#22-get_pte函数将页表项的查找和页表项的分配合并在一个函数的分析)
  - [练习3：给未被映射的地址映射上物理页（需要编程）](#练习3给未被映射的地址映射上物理页需要编程)
    - [3.1 `do_pgfault`函数实现分析](#31-do_pgfault函数实现分析)
    - [3.2 页目录项和页表项中组成部分的潜在用处。](#32-页目录项和页表项中组成部分的潜在用处)
    - [3.3 数据结构Page的全局变量每一项与页表中的页目录项和页表项的对应关系](#33-数据结构page的全局变量每一项与页表中的页目录项和页表项的对应关系)
...
在本实验中，我们主要比较了两种常见的页表映射方式：“一个大页”的页表和“分级页表”。这两种方法都用来管理虚拟内存和物理内存之间的映射，但它们在设计上有显著的差异。以下是这两种方式的优缺点分析。

### 1. **一个大页的页表映射方式**

"一个大页"的页表映射方式指的是在虚拟内存管理中，使用一个单级页表来管理所有的虚拟页与物理页之间的映射关系。在这种方式下，所有的虚拟地址映射都依赖于一个大的页表。

- **优点**：
  - **简化了页表结构**：一个大页表避免了多级查找，页表只需要一个层级，理论上可以减少访问页表的层级深度，简化了映射过程。
  - **内存访问速度更快**：由于没有多级页表的查找过程，一个大页的页表可以减少因多次查找而导致的内存访问延迟。
  - **适用于小内存系统**：在某些内存较小的嵌入式或特殊环境中，一个大页表可以更容易地实现，且管理简单。

- **缺点**：
  - **页表尺寸大**：如果虚拟地址空间较大，单级页表需要包含所有虚拟页面到物理页面的映射。由于每个页表项占用一定的字节，整个页表会变得非常庞大，尤其是对于32位或64位系统，页表会消耗大量内存。
  - **内存浪费**：即使某些虚拟页面没有映射到物理内存，也需要为它们在页表中预留位置，这导致内存的浪费。
  - **不适合大型系统**：在现代的操作系统和大型服务器中，虚拟地址空间极为庞大，单级页表的大小会非常庞大，不容易管理，甚至可能导致内存分配失败。

### 2. **分级页表映射方式**

分级页表映射方式使用多级页表结构，通常包括多个层级的页表，每一层的页表负责映射一部分虚拟地址空间。常见的如 x86 中的二级、三级或四级页表，在 RISC-V 中也有类似的结构（如 SV39、SV48）。

- **优点**：
  - **节省内存**：分级页表的每一层只在需要时才会分配内存，避免了预分配巨大的单一页表。大部分时候，只有部分页表会被填充，空闲的页表项不会占用内存。
  - **灵活性强**：每一级页表的大小是可变的，系统可以根据需要调整不同页表的大小。分页的层次结构也使得虚拟地址空间的管理更加灵活。
  - **高效的内存管理**：由于只需管理已使用的页表项，内存的利用率较高，而且系统对内存的管理更为精细。

- **缺点**：
  - **查找过程复杂**：由于有多个层级的页表，虚拟地址到物理地址的转换需要经过多次查找，可能增加每次地址转换的延迟。
  - **管理开销**：需要管理多个页表，增加了操作系统的实现复杂度。在某些情况下，操作系统需要在多个层级之间进行切换，增加了额外的开销。

### 3. **总结：比较“一个大页”与“分级页表”的优缺点**
...
| **查找速度**         | 快                | 较慢（需要多级查找）|
| **内存浪费**         | 高                | 低                  |
| **适用场景**         | 小内存、嵌入式    | 大内存系统、大型操作系统 |
| **灵活性**           | 低                | 高                  |
| **实现复杂度**       | 低                | 高                  |

- **"一个大页"的优点**在于其简单性和速度，适用于内存较小或者虚拟地址空间不大的场景，但它牺牲了内存的灵活性，适应不了大规模虚拟内存的需求。
- **"分级页表"的优点**在于节省内存并具有更高的灵活性，适合大内存系统，但其多级查找带来的性能开销在某些情况下可能成为瓶颈。

### 4. **结论**

选择"一个大页"还是"分级页表"应根据具体的硬件架构、操作系统需求和系统的内存管理目标来决定。在嵌入式系统或者资源受限的环境中，一个大页的映射方式可能更为高效；而在需要处理大量内存或支持大规模虚拟地址空间的系统中，分级页表则更具优势。因此，操作系统设计者需要权衡系统的需求，选择合适的分页方式。
